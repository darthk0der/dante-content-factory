...
export default function ReviewTab({ items, onUpdate, onPublish, onSchedule, onDelete }) {
  ...
  if (queue.length === 0) {
    return (
      <div className="empty-state">
        ...
        <div>Review queue is empty</div>
        <div style={{ fontSize: '12px', color: 'var(--muted)' }}>Generated content appears here for review before publishing</div>
      </div>
    );
  }
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: '12px', color: 'var(--muted)' }}>
                  {new Date(item.generated_at).toLocaleString('en-GB', { dateStyle: 'medium', timeStyle: 'short' })}
                </div>
              </div>
